...
| **Prometeo** | Prometheus "el que piensa antes", que planeó antes de actuar y dio las artes a la humanidad | Piensa adelante: planifica el trabajo antes de que nadie toque código | **Planificador** | `deepseek-flash` + `thinking` + `reasoning_effort=max` | plan: desglose de unidades, esfuerzo, presupuesto |
...
### Prometeo (Planificador) — el único rol con razonamiento premium
- Entradas: el objetivo, el mapa del código actual, el presupuesto, las restricciones activas.
- Salidas: desglose ordenado en unidades, esfuerzo estimado por unidad, riesgos, modelo propuesto por rol.
